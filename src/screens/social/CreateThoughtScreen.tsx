...
import { ThoughtForm } from '../../components/post/creation/forms/ThoughtForm';
...
export const CreateThoughtScreen = () => {
    ...
    const [selectedTopic, setSelectedTopic] = useState(draft?.data?.selectedTopic || '');
    ...
    const handleSaveDraft = async () => {
        try {
            if (draft) {
                await draftService.updateDraft(draft.id, { thoughtText: text, selectedTopic });
            } else {
                await draftService.saveDraft({ type: 'thought', data: { thoughtText: text, selectedTopic } });
            }
    ...
    return (
        <View style={[styles.container, { backgroundColor: theme.colors.background }]}>
            ...
            <View style={[styles.header, { paddingTop: insets.top + 10 }]}>
                ...
                <View style={{ width: 80 }}>
                    <Button onPress={handleSubmit} loading={isSubmitting} disabled={!selectedTopic} size="sm">
                        Paylaş
                    </Button>
                </View>
            </View>
