            ...
            try {
                ...
                if ($parentId) {
                    ...
                    if ($parentComment && $parentComment['user_id'] != $userId) {
                        ...
                        if ($notifStmt->execute()) {
                            if (isset($oneSignal)) {
                                $oneSignal->sendToUser($parentComment['user_id'], $title, $message, array("type" => "reply", "post_id" => $data->post_id));
                            }
                        } else {
                ...
                if ($postOwner) {
                    ...
                    if ($ownerId !== $senderId) {
                        ...
                        if ($notifStmt->execute()) {
                            if (isset($oneSignal)) {
                                $oneSignal->sendToUser($ownerId, $title, $message, array("type" => "comment", "post_id" => $data->post_id));
                            } else {
                                file_put_contents('../debug_log.txt', date('Y-m-d H:i:s') . " - OneSignal object not set in comment.php\n", FILE_APPEND);
                            }
                        } else {
